...
(defun count-through-range (start end filter fn)
  "Step through a range of numbers, applying fn to each one in turn."
  (labels ((step-through (start end acc)
	     "Step from start to end, accumulating the results of fn in acc."
	     (cond ((equalp start end) acc)
		   (T (let ((new-start (+ 1 start))
			    (accumulator (if (funcall filter start)
					     (cons (funcall fn start) acc)
					     acc)))
			(step-through new-start end accumulator))))))
    (nreverse (step-through start end '()))))
